module AdminForms
  module Helpers
    module Wrappers
      ADMIN_OPTIONS = [ :label, :help_inline, :error, :success, :warning, :help_block, :prepend, :append, :append_button, :control_group ]
    ...
    private
      def form_group_div(&block)
        ...
        klasses << 'has-error' if @field_options[:error]
        klasses << 'success' if @field_options[:success]
        klasses << 'warning' if @field_options[:warning]
        klasses << 'required' if @field_options.merge(required_attribute)["data-required"]

        control_group_options = {}
        control_group_options[:style] = @field_options.delete(:field_style)
      ...
      def input_div(&block)
        klass = @field_options.delete(:field_class) || (respond_to?(:options) && self.options[:field_class])
        content_options = {}
        content_options[:class] = 'controls'
        content_options[:class] = (content_options[:class] || "") + " " + klass if klass
        if @field_options[:control_group] == false
          @field_options.delete :control_group
          write_input_div(&block)
        else
          content_tag(:div, :class => content_options[:class]) do
            write_input_div(&block)
          end
        end
      end
      ...
      def write_input_div(&block)
        if @field_options[:append] || @field_options[:prepend] || @field_options[:append_button]
          klass = []
          klass << 'input-prepend input-group' if @field_options[:prepend]
          klass << 'input-append input-group' if @field_options[:append] || @field_options[:append_button]
      ...
      def label_field(&block)
        if @field_options[:label] == '' || @field_options[:label] == false
          ...
          label_options[:class] = (label_options[:class] || "") + " " + (@field_options[:label_class] || (respond_to?(:options) && self.options[:label_class])) if @field_options[:label_class] || (respond_to?(:options) && self.options[:label_class])
          
          if block_given?
            text = capture(&block)
          else
            text = @field_options[:label] || @name.to_s.humanize
            @field_options[:label] = text
            @field_options[:placeholder] = text unless @field_options.has_key?(:placeholder)
            text = ERB::Util.html_escape_once(text) unless text.html_safe?
          end
          text+= " <span class='required'>*</span>" if text.present? && @field_options.merge(required_attribute)["data-required"]
          if @field_options[:label_info].present?
            text+= "<br />" + content_tag(:small, @field_options[:label_info])
          end
          if respond_to?(:object)
             label(@name, text.html_safe, label_options)
           else
             label_tag(@name, text.html_safe, label_options)
           end
        end
      end
      ...
      def required_attribute
        ...
        if respond_to?(:object) and object.respond_to?(:errors) and object.class.respond_to?('validators_on')
          return { "data-required" => true } if object.class.validators_on(@name).any? { |v| is_presence_validator(v) }
        end
        {}
      end
      
      def is_presence_validator(v)
        (
      ...
      def valid_validator?(validator)
        action_validator_match?(validator) #&& !conditional_validators?(validator)
      end
      ...
      %w(help_inline error success warning help_block append append_button prepend).each do |method_name|
        define_method(method_name) do |*args|
          ...
          when 'append', 'prepend'
            content_tag(:span, value, :class => 'add-on input-group-addon')
          when 'append_button'
            if value.is_a? Array
