  ...
  def update_local_values
    ...
    Property.where(id: id).update_all(rooms: r, floor: f, floors: fs, address: a, state: s)
		...
		if _images.count > 0
			 ...
			 old_path = Rails.root.join('public', 'YandexDisk', 'photos', _district_old, info_old, self.id.to_s)
			 new_path = Rails.root.join('public', 'YandexDisk', 'photos', _district, info, self.id.to_s)
			...
			_images.each do |image|
				if image.image?
					...
					if File.exists?(old_path_with_filename) && old_path_with_filename != File.join(new_path, filename)
						...
						unless File.exists?(File.join(new_path, filename))
